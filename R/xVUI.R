...
xVUI <- function(id) {
  ns <- NS(id)
  tags$div(# script included below because subplots including plot_dendro have to be sized manually, unfortunately
    tags$script(sprintf("
      var widthId = '%s';
      var width = 0;
    ...
    withProgress(value = 0.2, message = "creating distance matrix...",
                 expr = {
                   tryCatch({
                     ...
                     # add new dendroplot
                     dendro <- plotly::plot_dendro(as.dendrogram(sample_clust), width = input$width)
                     dendro <- dendro %>% layout(xaxis = list(autorange = "reversed", showgrid = FALSE),
                                                 yaxis = list(showgrid = FALSE),
                                                 paper_bgcolor = "#FFFFFF", plot_bgcolor = "#FFFFFF",
                                                 margin = list(b = 0, t = 0, r = 25, l = 15))
