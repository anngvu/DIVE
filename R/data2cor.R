#' Get correlation/association matrix and observations used
#'
#' Get correlation/association matrix with Pearson or Spearman correlations
#'
...
#' (for interval vs ordinal and ordinal vs ordinal data).
...
#' such as Cramer's V, eta, lambda (something like data2association) so the result will be a heterogeneous
...
#' The default pattern excludes columns named "ID" and columns ending with "_SE", "_SEM", and "_SD".
#' @param type Either "pearson" or "spearman", defaults to "spearman".
#' @return A list with M, the correlation matrix, and N, the number of observations matrix, and P, the p-values matrix. See \code{\link[Hmisc]{rcorr}}.
#' @export
