...
matrixCtrlUI <- function(id) {
  ...
  tags$div(id = "matrixCtrlUI",
    fluidRow(
      column(12, 
             div(class = "forceInline", numericInput(ns("minN"), HTML("mininum <i>N</i>"), min = 2, max = NA, step = 1, val = 5, width = "80px")),
...
matrixCtrl <- function(input, output, session,
  ...
  default <- list(M = M, N = N, cdata = cdata, newdata = NULL, filM = M, filN = N)
  ...
  #-- Filtering handling ----------------------------------------------------------------------------------------------------#
  # Apply minimum N to current matrix
  # observe({
  #   has.n <- which(apply(state$N, 1, max) >= input$minN)
  #   
  # })
  ...
  observe({
    ...
    if(!nrow(applied)) {
      ...
      # match metadata terms to variables; the Variable optgroup is special since it potentially contains user-uploaded values,
      # i.e. be consistent with the workaround in opt()
      ...
      has.opt <- which(rownames(state$M) %in% has.opt)
      has.n <- which(apply(state$N, 1, max) >= input$minN) # exclude completely "dead" rows with no values above N
      keep <- intersect(has.n, has.opt)
       # modify M
      M2 <- state$M
      M2[state$N < input$minN] <- NA # gray out/remove r values calculated from less than specified n
      M2 <- M2[keep, keep]
      state$filM <- M2
    }
  })
